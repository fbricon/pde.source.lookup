...
import static org.jboss.tools.pde.sourcelookup.ui.internal.actions.PluginSourceLookupPropertyTester.isBinaryFragment;
import static org.jboss.tools.pde.sourcelookup.ui.internal.actions.PluginSourceLookupPropertyTester.isPluginContainer;
...
public class DownloadSourcesHandler extends AbstractHandler {
  ...
  private Collection<IPackageFragmentRoot> getSelectedClasspathEntries(ISelection selection) {
    ...
    for (Object o : structuredSelection.toList()) {
      if (o instanceof IPackageFragmentRoot) {
        ...
        if (isBinaryFragment(pfr) && belongsToPluginContainer(structuredSelection, pfr)) {
          fragments.add(pfr);
        }
  ...
  private Collection<IPackageFragmentRoot> filterPackageFragmentRoots(IAdaptable[] adaptables) {
    return Stream.of(adaptables).map(a -> a.getAdapter(IPackageFragmentRoot.class)).filter(pfr -> isBinaryFragment(pfr))
